      ...
      <main className="container-app py-12">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2 text-sm font-medium text-surface-500 mb-8">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-surface-900">{property.title}</span>
          </div>
          ...
          <div className="grid grid-cols-2 md:flex md:flex-row items-center justify-between gap-6 mb-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-bold text-xl text-surface-900">{property.bedrooms > 0 ? property.bedrooms : '-'}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-bold text-xl text-surface-900">{property.bathrooms > 0 ? property.bathrooms : '-'}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-bold text-xl text-surface-900">{property.area.toLocaleString()} <span className="text-sm font-medium text-surface-500">sqft</span></p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-bold text-xl text-surface-900">{property.parking > 0 ? property.parking : '-'}</p>
              </div>
            </div>
          </div>
          ...
          <section className="mb-12">
            ...
            {isAuthenticated ? (
              ...
              <div className="relative">
                <p className="text-surface-600 leading-relaxed text-lg blur-[2px] select-none">
                  {property.description}
                </p>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <div className="bg-white/90 backdrop-blur p-6 rounded-2xl shadow-lg border border-surface-200 text-center w-full max-w-sm">
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
              ...
              <div className="flex justify-between items-center border-b border-surface-100 pb-3">
                ...
                <span className="font-semibold text-surface-900">{property.furnishing}</span>
              </div>
              <div className="flex justify-between items-center border-b border-surface-100 pb-3">
                <span className="text-surface-500 font-medium">Age of Property</span>
                <span className="font-semibold text-surface-900">{property.age}</span>
              </div>
              <div className="flex justify-between items-center border-b border-surface-100 pb-3">
                <span className="text-surface-500 font-medium">Floor Level</span>
                <span className="font-semibold text-surface-900">{property.floor > 0 ? `${property.floor} of ${property.totalFloors}` : 'Ground'}</span>
              </div>
              {property.rera && (
                <div className="flex justify-between items-center border-b border-surface-100 pb-3 md:col-span-2">
                  ...
                  <span className="font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-lg">{property.rera}</span>
                </div>
              )}
            </div>
          </section>
        ...
        {lightboxOpen && (
          ...
          >
            <div className="flex justify-between items-center p-6 text-white">
              <span className="font-medium">{activeImage + 1} / {property.images.length}</span>
              <button onClick={() => setLightboxOpen(false)} className="hover:opacity-70 transition-opacity">
                <X className="w-8 h-8" />
              </button>
            </div>
              ...
              {property.images.length > 1 && (
                <>
                  <button 
                    ...
                    className="absolute left-6 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white backdrop-blur-md transition-colors"
                  >
                    <ChevronLeft className="w-6 h-6" />
                  </button>
                  <button 
                    onClick={() => setActiveImage(p => (p + 1) % property.images.length)}
                    className="absolute right-6 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white backdrop-blur-md transition-colors"
                  >
            ...
            {/* Thumbnail Strip */}
            {property.images.length > 1 && (
              <div className="p-6 overflow-x-auto flex justify-center gap-3">
                {property.images.map((img, i) => (
                  <button 
                    key={i} 
                    onClick={() => setActiveImage(i)}
                    className={`h-16 w-24 flex-shrink-0 transition-opacity rounded-lg overflow-hidden ${i === activeImage ? 'opacity-100 border-2 border-white' : 'opacity-40 hover:opacity-100'}`}
                  >
